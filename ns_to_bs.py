...
BS_MONTHS_ENG = ["Baisakh", "Jestha", "Ashad", "Shrawan", "Bhadra", "Ashwin", "Kartik", "Mangsir", "Poush", "Magh", "Falgun", "Chaitra"]
BS_MONTHS_NEP = ["वैशाख", "ज्येष्ठ", "आषाढ", "श्रावण", "भाद्र", "आश्विन", "कार्तिक", "मंसिर", "पौष", "माघ", "फाल्गुण", "चैत्र"]
BS_MONTHS_NEP_SHORT = ["वै.", "ज्ये.", "आ.", "श्रा.", "भा.", "आ.", "का.", "मं.", "पौ.", "मा.", "फा.", "चै."]

from base import (arabic_number_to_nepali, is_leap_year, verify_ns_date,
                  NS_DAYS_IN_MONTH, NS_DAYS_IN_MONTH_LEAP, NS_AD_OFFSET,
                  NS_MONTHS_NEP)
...
def convert_bs_to_ns(bs_year, bs_month, bs_date):
    ...
    if not is_valid_date:
        print(f"वि. सं. {arabic_number_to_nepali(bs_year)} {BS_MONTHS_NEP[bs_month - 1]} मा {arabic_number_to_nepali(bs_date)} दिन छैन")
        return

    total_bs_days = 0
    year = BASE_BS_YEAR_FOR_NS

    # Using the number of days in the given year (last element of the value in BS_CALENDAR_DATA)
    while year < bs_year:
...
def convert_ns_to_bs(ns_year, ns_month, ns_date):
    ...
    """
    if (
        ns_year < BASE_NS_YEAR_FOR_BS
        or ns_year == BASE_NS_YEAR_FOR_BS and ns_month < BASE_NS_MONTH_FOR_BS
        or ns_year == BASE_NS_YEAR_FOR_BS and ns_month == BASE_NS_MONTH_FOR_BS and ns_date < BASE_NS_DATE_FOR_BS
    ):
        print(f"Supported date range: Solar NS {BASE_NS_YEAR_FOR_BS}-{BASE_NS_MONTH_FOR_BS}-{BASE_NS_DATE_FOR_BS} to Solar NS 1164-6-29")
        return

    if ns_year > 1164 or ns_year == 1164 and ns_month > 6 or ns_year == 1164 and ns_month == 6 and ns_date > 29:
        print(f"Supported date range: Solar NS {BASE_NS_YEAR_FOR_BS}-{BASE_NS_MONTH_FOR_BS}-{BASE_NS_DATE_FOR_BS} to Solar NS 1164-6-29")
        return

    is_valid_date = verify_ns_date(ns_year, ns_month, ns_date)
    if not is_valid_date:
        print(f"सौ. ने. सं. {arabic_number_to_nepali(ns_year)} {NS_MONTHS_NEP[ns_month - 1]} मा {arabic_number_to_nepali(ns_date)} दिन छैन")
        return
    ...
    return f"{res_bs_year} {res_bs_month} {res_bs_date}"
